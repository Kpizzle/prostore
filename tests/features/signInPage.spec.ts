...
test.describe(
  ...
  () => {
    ...
    test('Error message displayed for invalid credentials ', async ({
      page,
    }) => {
      const signInPage = new SignInPage(page);
      await signInPage.completeSignInProcess(
        'wrongemail@domain.con',
        'incorrectPassword'
      );
      await expect(signInPage.invalidCredentialsText).toHaveText(
        'Unable to verify login credentials'
      );
    });

    test('Validation error displayed when submitting only email address', async ({
      page,
    }) => {
      const signInPage = new SignInPage(page);
      ...
      const validationMessage = await signInPage.getValidationMessage(
        signInPage.passwordInput
      );
      expect(validationMessage).toContain('Please fill out this field.');
    });

    test('Validation error displayed when submitting only a password', async ({
      page,
    }) => {
      const signInPage = new SignInPage(page);
    ...
    test('Validation error displayed when using an invalid email address', async ({
      page,
    }) => {
      const signInPage = new SignInPage(page);
